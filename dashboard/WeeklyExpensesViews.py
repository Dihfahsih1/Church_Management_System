...
#current week expenses on lunch
def total_current_week_lunch():
    lunch = Expenditures.objects.filter(Date__in=current_week_dates, Archived_Status='NOT-ARCHIVED').aggregate(lunch=Sum('Lunch'))
    lunch=lunch['lunch']
    if lunch == None:
        lunch = 0
    return lunch
...
#current week expenses on transport
def total_current_week_transport():
    transport = Expenditures.objects.filter(Date__in=current_week_dates, Archived_Status='NOT-ARCHIVED').aggregate(transport=Sum('Transport'))
    transport=transport['transport']
    if transport == None:
        transport = 0
    return transport
    
#current week expenses on Renovation
def total_current_week_renovation():
    renovation = Expenditures.objects.filter(Date__in=current_week_dates, Archived_Status='NOT-ARCHIVED').aggregate(renovation=Sum('Renovation'))
    renovation=renovation['renovation']
    if renovation == None:
        renovation = 0
    return renovation
    
#current week expenses on Stationery
def total_current_week_stationery():
    stationery = Expenditures.objects.filter(Date__in=current_week_dates, Archived_Status='NOT-ARCHIVED').aggregate(stationery=Sum('Stationery'))
...
def total_current_week_expenses():
    ...
    tot_lunch = total_current_week_lunch()
    tot_transport = total_current_week_transport()
    tot_renovation = total_current_week_renovation()
    tot_stationery = total_current_week_stationery()
    
    total_main = total_current_week_main()
    
    
    current_week_total_expenses = (total_tot + total_help + total_allowances+ total_others + total_bills + total_savings + total_love + total_petty + total_salaries + total_main+tot_lunch+tot_renovation+tot_transport+tot_lunch)
