...
#current month expenses on lunch
def total_current_month_lunch():
    lunch = Expenditures.objects.filter(Date__month=current_month, Archived_Status='NOT-ARCHIVED').aggregate(lunch=Sum('Lunch'))
    lunch=lunch['lunch']
    if lunch == None:
        lunch = 0
    return lunch
...
#current month expenses on transport
def total_current_month_transport():
    transport = Expenditures.objects.filter(Date__month=current_month, Archived_Status='NOT-ARCHIVED').aggregate(transport=Sum('Transport'))
    transport=transport['transport']
    if transport == None:
        transport = 0
    return transport
    
#current month expenses on Renovation
def total_current_month_renovation():
    renovation = Expenditures.objects.filter(Date__month=current_month, Archived_Status='NOT-ARCHIVED').aggregate(renovation=Sum('Renovation'))
    renovation=renovation['renovation']
    if renovation == None:
        renovation = 0
    return renovation
    
#current month expenses on Stationery
def total_current_month_stationery():
    stationery = Expenditures.objects.filter(Date__month=current_month, Archived_Status='NOT-ARCHIVED').aggregate(stationery=Sum('Stationery'))
...
def total_current_month_expenses():
    ...
    tot_lunch = total_current_month_lunch()
    tot_transport = total_current_month_transport()
    tot_renovation = total_current_month_renovation()
    tot_stationery = total_current_month_stationery()
    
    
    current_month_total_expenses = (total_tot + total_help + total_allowances + total_others + total_bills + total_savings + total_love + total_petty + total_salaries + total_main + tot_lunch + tot_renovation + tot_transport  + tot_stationery)
